...
- `src/types/database.generated.ts` will hold generated database types after the
  first schema review.
...
Phase 8A and Phase 8B are complete. Phase 8C is the next backend slice.

Do not add secrets, copied environment values, access tokens, or project IDs to
these files. Keep migrations and generated database types in their own source
paths once Supabase work starts.
