...
Worth noting up front: this is the same author behind `@web-kits/audio`, the sound library already wired into `button.tsx`/`checkbox.tsx`/`switch.tsx`/`toggle.tsx`/`toggle-group.tsx`/`radio-group.tsx`/`sonner.tsx` this session, and its "Minimal" sound patch. His writing on secondary action (post 2, below) directly explains the reasoning behind a system this project already ships.
...
- **Fitts's Law** — target acquisition time is a function of size and distance. His stated application is, almost word for word, the technique already shipped this session: **"expand button hit areas using invisible padding via `::before`/`::after` pseudo-elements."** That's exactly what `button.tsx`, `checkbox.tsx`, `switch.tsx`, and `radio-group.tsx` do via the `after:-inset-*` pattern. Good to have the formal name and reasoning for a technique this codebase already applies.
...
Technical reference for Motion's `AnimatePresence`, which this codebase already uses (`tabs.tsx`, `shared-layout-background.tsx`) — but only the `custom` prop so far. Worth having the fuller API on record:
...
- **`propagate` prop** — makes exit animations cascade into nested `AnimatePresence` children instead of those children just vanishing instantly. Not currently relevant: `shared-layout-background.tsx`'s per-item `AnimatePresence` blocks are independent, not nested inside each other.
- **`mode` prop** — `sync` (default, simultaneous), `wait` (sequential — the author notes this roughly doubles perceived duration since nothing overlaps), `popLayout` (removes exiting elements from layout flow immediately via absolute positioning, so siblings don't wait for an exit animation to finish before reflowing).
...
Checked against this session's actual choices (also cross-checked against Guri's `learn-spring-motion` post, which stated the same split): `tabs.tsx`, `shared-layout-background.tsx`, and `switch.tsx` press feedback are all pointer/interaction-driven → spring. Dialogs/sheets are system-announced open/close → duration-based. Both authors' framing lines up with what's already shipped, independently, twice now — this is the strongest, most corroborated single principle to come out of this research so far.
...
- **`::before`/`::after` for hover feedback layers**: his example animates a button's `::before` from `scale(0.95)`/`opacity: 0` to `scale(1)`/`opacity: 1` on hover — the same "never scale from 0" principle already in `make-interfaces-feel-better`, applied to a *pseudo-element hover background* rather than a real DOM node. Worth noting as a lighter-weight alternative for a **single, non-shared** hover-background effect — it can't replace `shared-layout-background.tsx`'s technique, since Motion's `layoutId` shared-layout animation needs a real React-managed DOM node to track, not a pseudo-element. Different tool for a different scale of problem: pseudo-element for one element's own hover state, `SharedLayoutBackground` for a highlight that moves *between* elements.
