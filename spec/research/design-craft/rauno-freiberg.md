...
- **Bypass React reconciliation for high-frequency interactions**: manipulate the DOM directly via a ref instead of routing every frame through state, specifically to hold 60fps. This is exactly the technique used in this session's own `switch.tsx` press-feedback work — `animate()` on a ref, no state, specifically because the no-useEffect skill's Rule 3 (event handlers, not state-flag relays) already pushed toward avoiding a state round-trip for that same reason. Good independent confirmation the switch.tsx approach wasn't just avoiding `useEffect` for its own sake — it's also the higher-performance choice for anything pointer-driven.
