...
His framing is about designer/developer fluency across a role divide, which doesn't map directly onto this project's workflow — but the underlying point still applies: reach for real CSS Grid (including subgrid) when a layout has genuine custom structure, rather than defaulting to a fixed column count or forcing something flexible into a rigid frame. Directly relevant to a decision already made this session: the original dimi.me `SharedLayoutBackground` trick used `grid-cols-subgrid` baked into the component itself, which was deliberately stripped out when adapting it (see `dimi.md`) on the grounds that a shared primitive shouldn't presume a specific grid structure before a real consumer needs one. This post is confirmation that the instinct was right on both ends of that decision: don't bake Grid assumptions into a generic primitive, *do* reach for real Grid once an actual layout with real structural needs exists — which hasn't happened yet, since no real pages are built.
...
1. **One font family by default; if you must pair two, pair serif with sans-serif, not two of the same category.** His stated failure mode is specifically two similar sans-serifs fighting each other. **This is directly relevant right now**: `styles.css` currently sets `--font-body` and `--font-heading` to the same typeface (Geist Variable), on record as an explicit placeholder — the project intends to introduce a distinct heading typeface later. This rule doesn't block that decision, but it's a real constraint worth having on hand when it happens: whatever heading font gets chosen, pairing a serif display face against Geist's sans body would be the "safe" combination by this rule; picking a second sans-serif risks the exact failure mode he describes, and would need more deliberate justification than a serif pairing would.
...
4. **Use weight contrast deliberately** — bold or light, rarely plain "regular," since subtle weight differences are easy for a reader to miss entirely. Geist Variable already supports fine-grained weight control, so this is immediately usable without any new tooling.
