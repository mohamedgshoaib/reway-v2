...
- Added `ThemeToggle` under `src/components/ui` and mounted it in the `/ui` audit header; moved `SoundProvider` into `src/components/providers`, added `ThemeProvider` there, and replaced the legacy theme module/hotkey components with a `useSyncExternalStore` theme store supporting persisted light, dark, and live system preferences.
...
- Theme preference is globally owned by `ThemeProvider`; `/ui` only renders the shared control. `system` is a persisted third preference that reacts to operating-system changes, while first-time users still default to light per the design-language contract.
