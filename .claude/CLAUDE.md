...
TanStack Start changes quickly. Before writing framework-specific code, use the `ctx7` CLI to read the current TanStack Start or TanStack Router docs, then check the installed package types and existing patterns in this repo. Do not carry over Next.js APIs or conventions.
...
- Use a shared image component when one exists; otherwise give `<img>` useful alt text and explicit dimensions

### Framework-Specific Guidance

**TanStack Start:**

- Define file routes in `src/routes`. Never edit `src/routeTree.gen.ts`
- Remember that route loaders and `beforeLoad` may run on the server during the first request and in the browser during later navigation. Keep secrets and server-only imports inside server functions or server route handlers
- Use `createServerFn` for application server work and route `server.handlers` for HTTP endpoints
- Define metadata with route `head`; render it through `HeadContent` in the root route
- Configure the framework in `vite.config.ts`, with `tanstackStart()` before `viteReact()`
...
- Fix UI root causes in `src/styles.css` or `src/components/ui/*`, never one-off page overrides, inline styles, or per-component patches.
