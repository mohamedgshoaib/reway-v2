...
import { DurableWorkerError } from "./durable-worker-error"
import type {
  ClaimedDurableWork,
  DurableClaimResult,
  DurableEnvelope,
...
const databaseFailure = (error: PostgrestError): DurableWorkerError =>
...
const invalidResult = (): DurableWorkerError =>
  new DurableWorkerError(
...
const requireTimestamp = (value: string): number => {
  ...
  if (!Number.isFinite(timestamp)) throw invalidResult()
  return timestamp
}
...
export const createSupabaseDurableWorkerAdapter = <Result>(
  ...
  claim: async (
    ...
    const response =
      request.functionName === "worker_claim_enrichment_message"
        ...
        : await client.rpc(request.functionName, request.parameters)
    if (response.error) throw databaseFailure(response.error)
    return parseClaim(response.data)
  },
  deleteTerminal: async (queueName, messageId): Promise<boolean> => {
  ...
  finish: async (claim, outcome, retryAtMs): Promise<DurableFinishState> =>
  ...
  rejectPoison: async (queueName, message, reason): Promise<boolean> => {
  ...
  renew: async (claim, leaseSeconds, visibilitySeconds): Promise<boolean> => {
  ...
  startAttempt: async (claim): Promise<boolean> => {
