...
import {
  type StateSurfaceAxis,
  stateSurfaceVariants,
} from "@/components/ui/state-surface"
import { cn } from "@/lib/utils"

export const toggleVariants = cva(
  "inline-flex shrink-0 items-center justify-center gap-2 rounded-lg border text-base font-medium whitespace-nowrap text-foreground transition-[scale,box-shadow] duration-150 ease-out-strong outline-none select-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-64 data-pressed:text-accent-foreground sm:text-sm pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 [&_svg]:pointer-events-none [&_svg]:-mx-0.5 [&_svg]:shrink-0 [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [:active,[data-pressed]]:scale-97",
  {
    defaultVariants: {
      size: "default",
      variant: "default",
    ...
    variants: {
      ...
      variant: {
        default:
          "border-transparent before:rounded-[calc(var(--radius-lg)-1px)] hover:before:bg-accent data-pressed:before:bg-input/64",
        outline:
...
export function Toggle({
  ...
  return (
    <TogglePrimitive
      className={cn(
        stateSurfaceVariants({ axis: surfaceAxis }),
        toggleVariants({ className, size, variant })
      )}
      data-slot="toggle"
